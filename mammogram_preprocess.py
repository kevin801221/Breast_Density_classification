...
if __name__ == "__main__":
    ...
    ap.add_argument("-i", "--input", type=str, required=True, help="path to the input folder containing images")
    ap.add_argument("-o", "--output", type=str, required=True, help="path to the output folder for processed images")
    ...
    for img_path in Path(input_folder).glob("*.png"):  # 改为所需的图像格式（例如：*.jpg, *.jpeg, *.tif等）
        # Read the image using OpenCV
        img = cv.imread(str(img_path), cv.IMREAD_GRAYSCALE)

        # Process the image
        processed_img, _, _ = process_image(img, sigma=3, skip_MR=skip)

        # Save the processed image to the output folder
        output_img_path = os.path.join(output_folder, img_path.name)
        cv.imwrite(output_img_path, processed_img)
